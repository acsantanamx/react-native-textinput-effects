...
export default class Fumi extends BaseInput {
  ...
  render() {
    ...
    const { focusedAnim, value, secureTextVisible } = this.state;
        ...
        <TextInput
          ...
          style={[
            styles.textInput,
            {
              ...
              marginRight: (secureTextEnabled) ? iconWidth + inputPadding : undefined,
              color: iconColor,
            },
            inputStyle,
          ]}
          ...
          secureTextEntry={(secureTextEnabled && secureTextVisible) ? true : false}
        />
        {secureTextEnabled &&
        <TouchableOpacity onPress={() => { this._onSecureTextStatus()}} 
              style={{
              position: 'absolute',
              right: inputPadding,
          ...
          <FontAwesomeIcon
            name={secureTextVisible ? 'eye-slash' : 'eye'}
            color="gray"
            size={iconSize}
